...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest req, HttpServletResponse res, Object handler) throws Exception {
		...
		// Step.2 Session Scope에 UserVO 객체가 공유되어 있다면 삭제처리
	...
	public void postHandle(HttpServletRequest req, HttpServletResponse res, Object handler, ModelAndView modelAndView) throws Exception {
		...
		// Step.1 ModelAndView 객체 안에, UserVO 가 있는지 확인
		ModelMap modelMap = modelAndView.getModelMap();
		UserVO userVO = (UserVO) modelMap.get(SharedScopeKeys.LOGIN_KEY);

		// Step.2 UserVO 가 있다면(로그인 성공했다면..), 
		...
		if(userVO != null) {	// 로그인 성공했다면...
			...
			if(isRememberMeOption) {	// 자동로그인 기능 on 해 놓았다면...				
				...
				boolean isUpdated = this.service.modifyUserWithRememberMe(userVO.getUserid(), rememberMeCookie.getValue(), expireTS);
